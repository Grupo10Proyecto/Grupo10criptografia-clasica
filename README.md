...
<body>
  ...
  <select id="algorithm">
    ...
    <option value="transposition">Transposición</option>
  ...
  <label for="key">Clave / Desplazamiento</label>
  ...
  <script>
    ...
    function caesarEncrypt(text, shift) {
      if (isNaN(shift)) return "Error: ingrese un número";
    ...
    function transpositionEncrypt(text, key) {
      ...
      return grid[0].map((_, i) => grid.map(r => r[i]).join(""))
        .join("");
    }
